...
{
	clk_sys_udelay(10000000); 	// Create 1 sec delay
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
	// Clear buffer and ensure UART TX & RX is free                //
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
	...
	clk_sys_udelay(100000); // Delay 0.1 second
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
	uint8_t token_cntr_flag = 0;
	int jj = 6;
	int token_cntr=0;
    unsigned long long token_start_t = 0;
    unsigned long long token_current_t = 0;

	token_start_t = get_ticks();
	while (token_cntr <= 1200 && token_current_t < 10)  // Make sure it doesn't wait more than 10 seconds
	{
		...
		if (uart0_status == QM_UART_RX_BUSY)
		{
		    ...
		    // Find the 'access' string
		    if (uart_rx[jj-6] == 'a' && uart_rx[jj-5] == 'c' && uart_rx[jj-4] == 'c' \
		     && uart_rx[jj-3] == 'e' && uart_rx[jj-2] == 's' && uart_rx[jj-1] == 's' )
		    {
			    token_cntr_flag = 1;
		    }

            // Count the number of token characters
		    if (token_cntr_flag == 1)
		    {
                token_cntr++;
		    }
		...
		token_current_t = (get_ticks() - token_start_t)/32000000;
	}
	QM_PRINTF("Token response time: %d\r\n", token_current_t);
...
void cellular_3g_send(const qm_uart_t uart, uint16_t device_id, uint8_t frame_cntr, uint8_t data_ch, uint16_t data_type, uint8_t data[], uint8_t data_len, uint8_t token_t[1200])
{
	uint8_t data_flag=0;
	//azwan added to check 200 or 400 respond
	for (int x=0;x<4;x++)
	{
		if (data_flag==0)
		{
			//azwan added. check if token is not available starts
			for (int tk=0;tk<50;tk++)
			{
				if(token_t[1]==0x00)
				{
					for (int token_loop=0;token_loop<4;token_loop++)
					{
						if (token_t[1]=='\0')
						{
							cellular_3g_get_token(QM_UART_0, token_t);
						}
					}
					break;
				}
			...
			//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
			//uint8_t token_t[1200];
			//memset(token_t,0,sizeof(token_t));
			//cellular_3g_get_token(QM_UART_0, token_t);
			...
			uint8_t cmd4_b[length_token_t];
			for(int jj=0; jj<length_token_t; jj++)
			{
				cmd4_b[jj] = token_t[jj];
			}
			...
			for(int jj=0; jj<16; jj++)
			{
				if (uart_rx[jj  ] == '2' && uart_rx[jj+1] == '0') // find 200 respond from server
				{
					QM_PUTS("200");
					data_flag=1;
				}

				else if (uart_rx[jj  ] == 'r' && uart_rx[jj+1] == 'e' && uart_rx[jj+ 2] == 'f' && uart_rx[jj+ 3] == 'r' && \
							uart_rx[jj+4] == 'e' && uart_rx[jj+5] == 's' && uart_rx[jj+6] == 'h')
				{
					QM_PUTS("refresh");
					data_flag=1;
				}
...
void cellular_rssi(const qm_uart_t uart)
{
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
	// Initialisations                                             //
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
...
uint8_t* cellular_3g_recv(const qm_uart_t uart, uint8_t payload_data[], uint8_t downlink_channel[], uint8_t token[1200])
{
	...
	uint8_t uart_rx[450];
	memset(uart_rx,0,sizeof(uart_rx));
	uint8_t payload_cntr_flag = 0;
	int jj = 4;
	int payload_cntr=0;
    unsigned long long payload_start_t = 0;
    unsigned long long payload_current_t = 0;

	payload_start_t = get_ticks();
	while (payload_cntr <= 30 && payload_current_t < 10)  // Maximum wait time is 10 seconds
	{
		// Get the data from UART Rx
	    qm_uart_get_status(uart, &uart0_status);
		if (uart0_status == QM_UART_RX_BUSY)
		{
		    ...
		    if (uart_rx[jj-4] == 'd' && uart_rx[jj-3] == 'a' && uart_rx[jj-2] == 't' && uart_rx[jj-1] == 'a')
		    {
			    payload_cntr_flag = 1;
		    }

            // Count the number of token characters
		    if (payload_cntr_flag == 1)
		    {
                payload_cntr++;
		    }
		...
		// Get the current time
		payload_current_t = (get_ticks() - payload_start_t)/32000000;

	}
	//QM_PRINTF("Payload response time: %d\r\n", payload_current_t);
