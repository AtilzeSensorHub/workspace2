...
{
	QM_PRINTF("Battery send to 3g1: %x\n", battery_level);
	QM_PRINTF("Battery send to 3g2: %x\n", battery_level);
	QM_PRINTF("Battery send to 3g3: %x\n", battery_level);

	uint16_t data[5];

	for(int x=0;x<3;x++)
	{
		data[x] = battery_level[x];
		//printf("battery_x: %x\n",battery_level[x]);
		QM_PRINTF("data_x: %x\n",data);
	}

	QM_PRINTF("battery_x0: %x\n",battery_level[0]);
	QM_PRINTF("battery_x1: %x\n",battery_level[1]);
	QM_PRINTF("battery_x2: %x\n",battery_level[2]);
	QM_PRINTF("battery_x3: %x\n",battery_level[3]);
