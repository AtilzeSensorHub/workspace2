...
void cellular_3g_send(const qm_uart_t uart, uint16_t device_id, uint8_t frame_cntr, uint8_t data_ch, uint16_t data_type, uint8_t data[], uint8_t data_len, uint8_t token[1200]);

void celluar_3g_send_status();

void cellular_3g_send_test(const qm_uart_t uart);
