...
void cellular_3g_init(qm_uart_config_t *cfg)
{
    ...
    // Send ATDL putsreq.com (set destination address)
    ...
    // Send ATAC (apply change without exit command mode)
    uint8_t at8[] = "ATAC\r\n";
...
uint8_t* cellular_3g_recv(const qm_uart_t uart, uint8_t payload_data[])
{
    // Commands to GET
	uint8_t cmd0[] = "GET /get HTTP/1.1\r\n";
	uint8_t cmd1[] = "Host: httpbin.org\r\n\r\n";

	qm_uart_write_buffer(uart, cmd0, (sizeof(cmd0)- 1));
	clk_sys_udelay(500000); // Delay 0.5 second
	qm_uart_write_buffer(uart, cmd1, (sizeof(cmd1)- 1));
	clk_sys_udelay(500000); // Delay 0.5 second
