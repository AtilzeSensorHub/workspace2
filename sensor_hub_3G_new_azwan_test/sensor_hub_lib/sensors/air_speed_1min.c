...
// Initalise PM25
void init_airspeed1min_control(qm_gpio_port_config_t *cfg)
{
			...
			//QM_PUTS("not idle \n");
				...
				if(uart_buffer == 's')
				{
					for (int k=0;k<3;k++)
					{
						...
						x[k]=uart_buffer;
						//QM_PRINTF("Value k: %d\n", k);
						QM_PRINTF("Data_buffer (air speed 1min): %d\n", uart_buffer);
					}
...
//	QM_PRINTF("haha: %d\n", haha);
	...
	airspeed1min_int=x[0];

	return airspeed1min_int;
